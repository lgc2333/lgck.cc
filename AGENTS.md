...
- Keep theme shell components such as `LgcSiteShell` free of `RouterView`; global page transition timing belongs in the `ValaxyApp.vue` override, but visual opacity/transform should target `LgcPageSurface` / `.lgc-page-surface` content areas instead of the whole route root.
